...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parses = partial_parses[:] # deep copy

    while unfinished_parses:
        minibatch = unfinished_parses[:batch_size]
        transitions = model.predict(minibatch) # each parser's transition extract

        for partial_parse, transition in zip(minibatch, transitions):
            partial_parse.parse_step(transition) # do transition

        unfinished_parses = [partial_parse for partial_parse in partial_parses
                             if partial_parse.buffer or len(partial_parse.stack) != 1] # if parser not finish, then unfinished_parser

...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # DO NOT use torch.nn.Linear or torch.nn.Embedding in pdf
        # W, b1
        self.embed_to_hidden_weight = nn.Parameter(torch.zeros([self.embed_size*self.n_features, self.hidden_size]), requires_grad=True)
        self.embed_to_hidden_bias = nn.Parameter(torch.zeros([self.hidden_size]), requires_grad=True)

        torch.nn.init.xavier_uniform_(self.embed_to_hidden_weight, gain=1)
        ...
        torch.nn.init.xavier_uniform_(self.hidden_to_logits_weight, gain=1)
    ...
    def embedding_lookup(self, w):
        ...
        # CS224n 2021 solution
        #          input                            output
        # (batch_size, n_features) -> (batch_size, n_features * embed_size)
        # embedding vector: (features   100 , embed_size 30)
        # words           : (batch_size 4   , n_features 36)
        # output          : (batch_size 4   , n_features*embed_size)

        embeddings = nn.Parameter(torch.tensor(self.embeddings))
        embeddings = torch.index_select(embeddings, 0, torch.flatten(w))
    ...
    def forward(self, w):
        ...
        hidden = F.relu(embedding@self.embed_to_hidden_weight + self.embed_to_hidden_bias)
        dropout = self.dropout.forward(hidden)
        logits = dropout@self.hidden_to_logits_weight + self.hidden_to_logits_bias

        # CS224n 2019 solution
        # hidden = F.relu(self.embed_to_hidden.forward(embedding))
        # print(hidden.shape)
        # dropout = self.dropout.forward(hidden)

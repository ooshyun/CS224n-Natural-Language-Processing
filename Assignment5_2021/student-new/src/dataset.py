...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncate_doc = document[:truncate_len]

        #2 break to [prefix][masked_content][suffix]
        masked_len = random.randint(int(1/8*truncate_len), int(3/8*truncate_len))  #1/4 the length of the truncated document on average.
        assert truncate_len >= 4, (doc_len, truncate_len, document, idx)
        prefix_len = random.randint(1, truncate_len - masked_len - 1)

        prefix = truncate_doc[:prefix_len]
        masked_content = truncate_doc[prefix_len: prefix_len + masked_len]
        suffix = truncate_doc[prefix_len + masked_len:]

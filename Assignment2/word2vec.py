...
def naiveSoftmaxLossAndGradient(
    ...
    # Goal is one-hot vector form
    y = np.zeros_like(y_hat)
    y[outsideWordIdx] = 1

    # Loss J(v_c, o, U) = log(P(O=0|C=c))
    loss = -np.log(y_hat[outsideWordIdx])

    # gradCenterVec dJ/dv_c = U @ (y_hat - y)
    gradCenterVec = outsideVectors.T@(y_hat - y)

    # gradOutsideVecs dJ/dU = (y_hat - y) * v_c^T
    # cs224n-2021-lecture03-nuralents.pdf 41 page
...
def negSamplingLossAndGradient(
    ...
    # J_{neg-sample}(v_c,o,U)= -log(sigmoid(u_o^T v_c))-sum(k[1:K+1])_log(sigmoid(-u_k^T v_c))
    loss = -np.log(sigmoidOutsideCenter) - np.log(sigmoidSamplesCenter).sum()

    # dJ/dv_c = (sigmoid(u_o^Tv_c)-1) * u_o + sum(k[1:K+1])(1-sigmoid(u_k^T v_c))@u_k
    # gradCenterVec = (sigmoidOutsideCenter - 1) * outsideWordVec - (sigmoidSamplesCenter -1).T @ samplesWordVecs
    # Above code is from reference. But sigmoidSamplesCenter.T = sigmoidSamplesCenter -> Why he use?
    gradCenterVec = (sigmoidOutsideCenter - 1) * outsideWordVec - (sigmoidSamplesCenter -1) @ samplesWordVecs

    # dJ/du_o = (sigmoid(u_o^Tv_c)-1)*u_o
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx, :] = (sigmoidOutsideCenter - 1) * centerWordVec
     
    # dJ/du_k = -(sigmoid(u_k^Tv_c)-1)*u_k
